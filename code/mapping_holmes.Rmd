---
title: "Base map for individual reefs - Holmes"
...
# First need to specify data from which to draw holmes Reef area data:
```{r read-holmes-data, message=FALSE, warning=FALSE}
holmes_reef <- 
...
holmes_base <- 
  ...
  coord_quickmap(ylim = c(-16.58, -16.35), xlim = c(147.75, 148.11)) +
...
holmes_vr2s <- 
  ...
  scalebar(holmes_reef, dist = 4, dist_unit = "km", transform = TRUE, model = "WGS84",            # Geo properties
           location = 'bottomleft', anchor = c(x = 147.75, y = -16.579),                                # Position of bar
           family = "Arial", st.size = 3.2, st.dist = 0.0007,                                            # Text properties
           height = 0.0005, border.size = 0.3, st.bottom = FALSE) +
    draw_image('../data/North_Pointer.jpg', x = 147.76, y = -16.37, hjust = 0.5, vjust = 0.5, scale = 0.03)
...
```{r holmes-sites}
ggsave(holmes_vr2s, filename = '../output/receivers_holmes.png', width = 200, height = 200, units = 'mm', dpi = 400)
```
