...
# And add back in the totals and also a variable that will dictate the bubble size in the end plot
```{r}
locsum<- locsum %>% 
...
locsum <- 
locsum %>% 
mutate(lat1 = case_when(grepl("Osprey", Reef) ~ -13.95,
                        grepl("Bougain", Reef) ~ -15.1,
                        grepl("Holmes", Reef) ~ -15.9,
                        grepl("Flind", Reef) ~ -17.25),
...
nreefsmap +
  ...
  scale_size(range = c(8, 53)) +
  scale_colour_brewer(palette = "Dark2") +
  geom_text(data = locsum, aes(x = long1+1.5 , y = lat1, label = Reef)) +
  geom_text(data = locsum, aes(x = long1, y = lat1, label = Total)) +
  theme(legend.position = 'none') +
  coord_quickmap(ylim = c(-17.8, -13.2), xlim = c(143, 152)) +
  ylab("Latitude")+
  xlab("Longitude")
...
straya <- 
  ...
  ggplot() +
  geom_polygon(aes(x = long, y = lat, group = group), colour = 'black', fill = 'grey40', lwd = 0.3) +
  theme(axis.title = element_blank(), 
        axis.text = element_blank(),
        axis.ticks = element_blank(),
        ...
        panel.background = element_rect(fill = "transparent"),
        plot.background = element_rect(fill = "transparent", colour = "transparent")) +
  geom_rect(aes(xmin = 143, xmax = 152, ymin = -18, ymax = -13), color = "white", fill = 'transparent', lwd = 0.1) +
...
final_tag_plot <- 
bubble_fig +
  annotation_custom(ggplotGrob(straya), xmin = 143.1, xmax = 145, ymin = -17.7, ymax = -16) +
  scalebar(cs_dryreef_df, dist = 100, dist_unit = "km", transform = TRUE, model = "WGS84", height = 0.008, 
           location = 'bottomleft', anchor = c(x = 143.1, y = -17.9), family = "Arial", st.size = 3.2, border.size = 0.4, st.bottom = FALSE) +
  draw_image('../data/North_Pointer.jpg', x = 143.6, y = -16, scale = 0.5)
final_tag_plot
```
...
# Save
```{r}
ggsave("../output/tagging_effort.png", plot = final_tag_plot, width = 220, height = 138, units = "mm")
```



# Bubble map V2
## Respecify lat1 and long1 points and add labels
```{r locsum2}
locsum2 <- 
  locsum %>% 
  mutate(lat1 = case_when(grepl("Osprey", Reef) ~ -14.2,
                        grepl("Bougain", Reef) ~ -15.5,
                        grepl("Holmes", Reef) ~ -15.9,
                        grepl("Flind", Reef) ~ -17.2),
       long1 = case_when(grepl("Osprey", Reef) ~ 148,
                         grepl("Bougain", Reef) ~ 146.6,
                         grepl("Holmes", Reef) ~ 148.5,
                         grepl("Flind", Reef) ~ 147)) %>% 
  mutate(label = c("b", "d", "c", "a"))
```

```{r bubble-fig2, fig.height=6, fig.width=10}
bubble_fig2 <- 
nreefsmap +
  #geom_polygon(data = gbr_dryreef_df, aes(x = long, y = lat, group = group), color = 'gray', fill = 'white', size = .2) +
  #geom_polygon(data = gbr_feats_df, aes(x = long, y = lat, group = group), color = 'gray', fill = 'white', size = .2) +
  #geom_polygon(data = gbr_shelf_df, aes(x = long, y = lat, group = group), color = 'gray', fill = 'white', size = .2) +
  geom_segment(data = locsum2, aes(x = long, y = lat, xend = long1, yend = lat1), size = .3,  col = "black") +
  geom_polygon(data = reefs_north, aes(x = long, y = lat, group = group), color = 'grey30', fill = 'grey', lwd = 0.3) + # reefs over top of lines
  geom_point(data = locsum2, aes(x = long1, y = lat1, size = Total), fill = "white", col = "black", stroke = 0.5, pch = 21) +
  scale_size(range = c(8, 53)) +
  scale_colour_brewer(palette = "Dark2") +
  #geom_text(data = locsum2, aes(x = long1+1.5 , y = lat1, label = Reef)) +
  geom_text(data = locsum2, aes(x = long1, y = lat1, label = Total)) +
  geom_text(data = locsum2, aes(x = long1, y = lat1 + 0.23, label = label), size = 4) +
  theme(legend.position = 'none') +
  coord_quickmap(ylim = c(-17.8, -13.2), xlim = c(143, 149)) +
  ylab("Latitude")+
  xlab("Longitude")
...
```{r final-tag-plot2, fig.width = 10, fig.height = 6, out.height =}
final_tag_plot2 <- 
bubble_fig2 +
  annotation_custom(ggplotGrob(straya), xmin = 143.1, xmax = 145, ymin = -17.7, ymax = -16) +
  scalebar(cs_dryreef_df, dist = 100, dist_unit = "km", transform = TRUE, model = "WGS84", height = 0.008, 
...
# Save V2
```{r}
ggsave("../output/tagging_effort2.png", plot = final_tag_plot2, width = 220, height = 180, units = "mm")
```
