...
map_data("world") %>% filter(region == 'Australia') %>% filter(lat > -25) %>% # E Coast
 ggplot() +
  ...
  geom_polygon(aes(x = long, y = lat, group = group), colour = 'grey10', fill = 'grey40', lwd = 0.5) +
...
#bigmov_final <- 
  bigmov_map +
 # annotation_custom(ggplotGrob(zoom_map), xmin = 151, xmax = 159.5, ymin = -16, ymax = -10) +
  annotation_custom(ggplotGrob(taggingmap),  xmin = 151, xmax = 159.5, ymin = -16, ymax = -10) +
