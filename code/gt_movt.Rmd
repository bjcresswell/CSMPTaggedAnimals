---
title: "GT movement timing analysis"
...
knit: (function(inputFile, encoding) { 
      ...
      rmarkdown::render(inputFile,
                        encoding=encoding, 
                        output_file=file.path(dirname(inputFile), out_dir, 'GT-movement-timing.html')) })
output: 
 html_document:
    code_folding: show
    collapse: no
...
gt_annual <- 
  ...
  dplyr::select(transmitter_id, station_name, detection_date) %>% 
  distinct() %>% 
...
```{r fig.width=16}
gt_annual %>% 
  ggplot(aes(x = detection_date, y = station_name)) + 
  xlab("Date") + 
  ylab("Detections") +
  geom_point() +
  #theme(axis.text.y = element_blank()) +
